...
```
Flags: `-n N` (max results), `--body` (include full body), `--json` (JSON output). Supports all Gmail search operators. Body extraction prefers `text/plain`; falls back to stripped `text/html` when no plain-text part exists (needed for HTML-only mail like Zoom's AI summary emails).
