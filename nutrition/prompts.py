...
LABEL_SCHEMA = {
    ...
    "properties": {
        ...
        "servings_per_package": {
            "type": "number",
            "description": (
                "How many declared servings the package contains, e.g. 5 for a 500 g bag "
                "with a 100 g serving. 1 if the package and the serving are the same thing "
                "(a single yogurt cup, a single bar). 0 if not stated."
            ),
        },
        "net_weight_grams": {
            "type": "number",
            "description": (
                "Total net weight/volume of the package as printed near the barcode or on "
                "the front (e.g. '500 g', '1 L', 'net wt 200g'), converting ml to g at 1:1 "
                "if it's a liquid. This is the whole-container amount and is independent of "
                "serving_grams - a 500 g bag with a stated 100 g serving has "
                "net_weight_grams 500 and serving_grams 100. 0 if genuinely not printed or "
                "not legible."
            ),
        },
...
- Protein specifically: before returning 0 for it, confirm there truly is no
  ...
  varies (see regional context above). 0 protein is correct only when the row is
